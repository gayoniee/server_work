...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 추출한 값(요청시 전달된 값)들을 가지고 요청처리를 해야함(DB와 상호작용)
		...
		// 위에서 요청 처리 후 성공했다는 가정하에 사용자가 보게될 응답페이지(html) 만들어서 전송
		// 즉, 여기 Java코드 내에 사용자가 보게될 응답html 구문을 작성할 것
		
		// 장점 : Java코드내에 작성하기 때문에 반복문이나 조건문, 유용한 메소드 활용가능
		// 단점 : 불편, 복잡, 혹시라도 차후에 html을 수정한다면 Java코드를 수정하는 것이기 때문에
		//		 수정된 내용을 다시 반영시키고자 서버를 재실행(restart) 해야함
		...
		out.println("h2{color:salmon}");
		out.println("#name{color:orange}");
		out.println("#age{color:skyblue}");
		out.println("#city{color:green}");
		out.println("#height{color:violet}");
